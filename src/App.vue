    ...
    <section class="panel settings">
      ...
      <div class="field">
        ...
        <div style="display:flex; gap:8px; align-items:center;">
          <input :value="sessionId" type="text" readonly />
          <button type="button" class="ghost" @click="copySessionId">Kopieren</button>
        </div>
        <div class="hint" style="margin-top:6px">Diese ID wird automatisch generiert und ist nicht änderbar.</div>
      </div>
      <div class="field">
...
const apiBase = ref(window.__APP_CONFIG__?.VITE_CHAT_API_BASE || '/api')
const apiPath = ref(window.__APP_CONFIG__?.VITE_CHAT_API_PATH || '/chat')
// session id is generated by the UI for each message history and not editable by the user.
...
const copySessionId = async () => {
  ...
  // Fallback: select input and execCopy not implemented here; it's best-effort only
}
...
const defaultTargets = [
  { id: 'broker', label: 'Broker', url: window.__APP_CONFIG__?.VITE_BROKER_URL || 'URL nicht gesetzt' },
  { id: 'clients', label: 'MCP Clients', url: window.__APP_CONFIG__?.VITE_MCP_CLIENT_URL || 'URL nicht gesetzt' },
  { id: 'llm', label: 'LLM', url: window.__APP_CONFIG__?.VITE_LLM_URL || 'URL nicht gesetzt' }
]
